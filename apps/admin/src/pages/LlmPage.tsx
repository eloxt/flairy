...
const ROLES: { role: LlmRole; label: string; description: string; clearable: boolean }[] = [
  {
    role: 'main',
    label: 'Main model',
    description: 'Drives the agent loop. Required — clients cannot run without it.',
    clearable: false
  },
  {
    role: 'tool',
...
function emptyProviderForm(): ProviderForm {
  return { id: '', name: '', provider: 'anthropic', credential: '', baseUrl: '' }
}
...
function modelToForm(m: LlmModelConfig): ModelForm {
  return { id: m.id, providerId: m.providerId, name: m.name, model: m.model }
}

function emptyModelForm(defaultProviderId: string): ModelForm {
  return { id: '', providerId: defaultProviderId, name: '', model: '' }
}
...
export function LlmPage(): React.JSX.Element {
  ...
  const [editingProvider, setEditingProvider] = useState<ProviderForm | null>(null)
  ...
  async function run(fn: () => Promise<unknown>, done: () => void): Promise<void> {
    try {
      await mutate(fn)
      done()
    } catch {
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => setEditingProvider(emptyProviderForm())}>
                ...
                <TableBody>
                  {providers.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-muted-foreground">{p.baseUrl || '—'}</TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => void run(() => deleteLlmProvider(p.id), () => {})}
                            disabled={saving}
                          >
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            onClick={() => setEditingModel(emptyModelForm(providers[0]?.id ?? ''))}
          >
            <Plus className="size-4" />
            Add model
          </Button>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => void run(() => deleteLlmModel(m.id), () => {})}
                            disabled={saving}
                          >
                            <Trash2 className="size-4" />
                          </Button>
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Which model each scenario uses. These assignments are delivered to every client.
          </p>
        </div>
        <Card>
          <CardContent className="space-y-5 p-6">
            {models.length === 0 ? (
              <p className="text-sm text-muted-foreground">Add a model before assigning roles.</p>
            ) : (
                  ...
                  <div key={role} className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Select
                        ...
                        onValueChange={(v) => {
                          if (v) void run(() => assignLlmRole(role, v), () => {})
                        }}
                        disabled={saving}
                      >
                      ...
                      {clearable && assigned && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => void run(() => clearLlmRole(role), () => {})}
                          disabled={saving}
                        >
                          Clear
                        </Button>
        ...
        <ProviderEditor
          ...
          onSubmit={() =>
            void run(
              () =>
                editingProvider.id
                  ? updateLlmProvider(editingProvider.id, providerFormToInput(editingProvider))
        ...
        <ModelEditor
          ...
          onSubmit={() =>
            void run(
              () =>
                editingModel.id
                  ? updateLlmModel(editingModel.id, modelFormToInput(editingModel))
...
function ProviderEditor({
  ...
  return (
    <Modal title={form.id ? `Edit ${form.name}` : 'New provider'} onClose={onCancel}>
      ...
      <div className="space-y-2">
        ...
        <Select value={form.provider} onValueChange={(v) => patch({ provider: v as LlmProvider })}>
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">
          Shared by every model under this provider and delivered to every client. Prefer a scoped /
          short-lived token over a long-lived master key.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">Gateway / proxy override.</p>
      </div>

      <EditorActions saving={saving} valid={valid} onCancel={onCancel} onSubmit={onSubmit} />
    </Modal>
  )
}
...
function ModelEditor({
  ...
  const valid =
    form.name.trim().length > 0 && form.model.trim().length > 0 && form.providerId.length > 0

  return (
    <Modal title={form.id ? `Edit ${form.name}` : 'New model'} onClose={onCancel}>
      <div className="space-y-2">
