...
import { createSystemPrompt, deleteSystemPrompt, updateSystemPrompt } from '@/api/client'
...
function promptToInput(prompt: SystemPromptConfig, enabled: boolean): SystemPromptInput {
...
export function SystemPromptsPage(): React.JSX.Element {
  ...
  async function handleSubmit(): Promise<void> {
    ...
    try {
      await mutate(() =>
        editing.id ? updateSystemPrompt(editing.id, input) : createSystemPrompt(input)
      )
      setEditing(null)
    } catch {
  ...
  async function handleToggle(prompt: SystemPromptConfig, enabled: boolean): Promise<void> {
    try {
      await mutate(() => updateSystemPrompt(prompt.id, promptToInput(prompt, enabled)))
    } catch {
      // surfaced via hook error state
    }
  }
    ...
    <Dialog open onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="max-h-[90vh] gap-0 overflow-auto sm:max-w-lg">
        <DialogHeader className="mb-4">
          <DialogTitle>{form.id ? `Edit ${form.name}` : 'New system prompt'}</DialogTitle>
        </DialogHeader>
