...
import { useUploadSkillFile } from './queries'
...
function getSourceOption(sourceType: string): (typeof FILE_SOURCE_OPTIONS)[number] {
  return (
    FILE_SOURCE_OPTIONS.find((option) => option.value === sourceType) ?? FILE_SOURCE_OPTIONS[0]!
  )
}
...
/** Read a File into a data URL (base64) so its bytes can be sent inline. */
...
function FileAddForm({
  ...
  onCancel,
  className
}: FileAddFormProps): React.JSX.Element {
  const uploadSkillFile = useUploadSkillFile()
  ...
  const [url, setUrl] = useState(initialEntry?.sourceUrl ?? '')
  const [content, setContent] = useState(initialEntry?.content ?? '')
  const [dataurl, setDataurl] = useState(initialEntry?.dataurl ?? '')
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [mimeType, setMimeType] = useState(initialEntry?.mimeType ?? 'text/plain')
  ...
  const handleUploadFileChange = (file: File | null): void => {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    const entry: SkillFileEntry = {
      path: fullPath,
      sourceType: sourceType as SkillFileEntry['sourceType'],
      mimeType,
      fileSizeBytes: selectedFile?.size ?? initialEntry?.fileSizeBytes,
    ...
    if (sourceType === 'upload') {
      ...
      try {
        // Validate server-side (size/MIME) and read bytes inline for the payload.
        const upload = await uploadSkillFile.mutateAsync(selectedFile)
        entry.dataurl = await readFileAsDataUrl(selectedFile)
        entry.mimeType = upload.mimeType || mimeType
        entry.fileSizeBytes = upload.fileSizeBytes
        entry.__local = true
      } catch (err: unknown) {
    ...
    if (sourceType === 'url') entry.sourceUrl = url
    ...
    <div className={cn('w-full rounded-sm border border-dashed p-2 flex flex-col gap-3', className)}>
      ...
      <div className="flex justify-end gap-2 pt-1">
        ...
        <Button
          ...
          onClick={() => void handleSubmit()}
          disabled={uploadSkillFile.isPending}
        >
...
function getMovedPath(path: string, fromFolder: string, toFolder: string): string {
...
function buildTree(files: SkillFileEntry[]): TreeFolder {
...
function addChildNode(
  ...
  if (node.data.path === parentPath) {
    node.children =
      placement === 'start' ? [child, ...(node.children ?? [])] : [...(node.children ?? []), child]
    return true
  }
...
export function FileManagerSection({
  ...
  const [editingFileOriginal, setEditingFileOriginal] = useState<{ path: string } | null>(null)
  const [searchQuery, setSearchQuery] = useState('')
  const [addingFile, setAddingFile] = useState<{ folderPath: string; sourceType: string } | null>(
    null
  )
  ...
  const expandFolder = (folderPath: string): void => {
  ...
  const treeData = useMemo(() => {
    ...
    const addEmptyFolderDrafts = (node: TreeNode<FileTreeNodeData>): void => {
  ...
  const handleFolderUploadClick = (folderPath: string): void => {
  ...
  const handleAddFileUploadClick = (folderPath: string): void => {
    if (isFolderUploading) return
  ...
  const handleSingleFileUploadChange = async (
    ...
    try {
      const upload = await uploadSkillFile.mutateAsync(file)
  ...
  const handleFolderUploadChange = async (
    ...
    try {
      ...
      setFolderUploadError(err instanceof Error ? err.message : 'Upload failed')
    } finally {
      setFolderUploadState(null)
    }
  }

  const moveFileToFolder = (index: number, folderPath: string): void => {
  ...
  const moveFolderToFolder = (folderPath: string, targetFolderPath: string): void => {
  ...
  const removeFolder = (folderPath: string): void => {
    setFolders((prev) =>
      prev.filter((folder) => folder !== folderPath && !folder.startsWith(`${folderPath}/`))
    )
    ...
    const adjustIndexAfterBatchRemove = (currentIndex: number | null): number | null => {
    ...
    if (addingFile?.folderPath === folderPath || addingFile?.folderPath.startsWith(`${folderPath}/`))
      setAddingFile(null)
  ...
  const requestRemoveFolder = (folderPath: string): void => {
  ...
  const handleAdd = (entry: SkillFileEntry): void => {
  ...
  const addFolder = (parentPath: string): void => {
    setNewFolderError(null)
    ...
    if (
      ...
      files.some((file) => file.path === nextPath || file.path.startsWith(`${nextPath}/`))
    ) {
  ...
  const renderItem = ({
    ...
    if (item.kind === 'file' && item.file && item.fileIndex != null) {
      ...
      const fileMoveTargets = availableFolderPaths.filter(
        (folderPath) => folderPath !== dirname(file.path)
      )
      const fileMeta = [
        file.sourceType,
        file.mimeType,
        file.fileSizeBytes ? formatFileSize(file.fileSizeBytes) : null
      ]
        .filter(Boolean)
        .join(' · ')
        ...
        >
          <div
            className={cn(
              ...
              onSelectFile && !isRenaming && 'cursor-pointer'
            )}
            ...
            >
              <DropdownMenu onOpenChange={(open) => setActiveDropdownNodeId(open ? item.id : null)}>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  {fileMeta && (
                    <>
                      <DropdownMenuLabel className="text-muted-foreground text-xs font-normal break-all">
                        {fileMeta}
                      </DropdownMenuLabel>
                      <DropdownMenuSeparator />
                    </>
                  ...
                  <DropdownMenuItem
                    ...
                    className="cursor-pointer"
                    onClick={() =>
                      setFileToRemove({ index, path: file.path, isLocal: !!file.__local })
                    }
                  >
                    Delete
                  </DropdownMenuItem>
    ...
    return (
      <div
        ...
        aria-label={
          hasChildren ? `${isExpanded ? 'Collapse' : 'Expand'} ${isRoot ? 'root' : item.name}` : undefined
        }
      >
        {hasChildren ? (
          <span
            className="text-muted-foreground flex h-4 w-4 items-center justify-center"
            aria-hidden="true"
          >
            {isExpanded ? (
                ...
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent>
                    <DropdownMenuItem
                      ...
                      onClick={() => handleAddFileUploadClick(item.path)}
                    >
                      Upload
                    </DropdownMenuItem>
                    <DropdownMenuItem
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {folderDeleteImpact?.nestedFiles.length ? (
                <>
                  This will remove the folder <b>{folderToDelete}</b>, its nested folders, and all
                  files inside it from this skill draft.
                </>
              ) : folderDeleteImpact?.nestedFolders.length ? (
                <>
                  This will remove the folder <b>{folderToDelete}</b> and its nested folders from this
                  skill draft. There are no files in the hierarchy of this folder.
                </>
              ) : (
                <>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {fileToRemove?.isLocal ? (
                ...
                <>
                  This will remove <b>{fileToRemove?.path}</b> from this skill draft. The file stops
                  being tracked only after you save these changes.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
