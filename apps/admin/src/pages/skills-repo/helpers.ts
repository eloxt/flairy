import type { SkillInput } from '@flairy/shared'
...
export function useSkillForm(initial?: SkillFormState) {
  ...
  const getPayload = (): SkillInput => {
    ...
    return {
      ...
      extraFrontmatter: parsedExtraFrontmatter,
      allowedTools: allowedTools || undefined,
      skillMdBody,
      enabled,
      files: files.map(({ __local, ...file }) => {
        void __local
        return file
      })
    }
  ...
  const addFile = (entry: SkillFileEntry): void => setFiles((prev) => [...prev, entry])
  const removeFile = (index: number): void =>
    setFiles((prev) => prev.filter((_, i) => i !== index))
  const updateFile = (index: number, updates: Partial<SkillFileEntry>): void =>
    setFiles((prev) => prev.map((f, i) => (i === index ? { ...f, ...updates } : f)))

  const reset = (state: SkillFormState): void => {
    setName(state.name)
