/**
 * Image format detection by magic bytes. Ported from pi-agent-core
 * (harness/tools/image.ts); the imageProcessor/auto-resize pipeline is not
 * ported (Flairy inlines images directly with a size cap — see read.ts).
 *
 * Detection is content-based, not extension-based, so files with a wrong or
 * missing extension are still recognized. Unsupported variants are rejected:
 * JPEG-XL-in-JPEG (0xFF 0xD8 0xFF 0xF7), animated PNG, and malformed BMP all
 * return undefined so they fall through to the text path or the caller's
 * "omitted" note instead of being sent to a provider that can't decode them.
 */
