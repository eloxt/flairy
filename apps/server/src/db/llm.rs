...
/// Parse the `input_modalities` TEXT[] column into `Vec<Modality>`, dropping any
/// unknown element (the CHECK constraint keeps these to {text,image}). Falls back
/// to `[Text]` if the row somehow has no recognizable modality, so the field is
/// never empty downstream.
fn parse_modalities(raw: Vec<String>) -> Vec<Modality> {
    let parsed: Vec<Modality> = raw.iter().filter_map(|s| Modality::from_str(s)).collect();
    if parsed.is_empty() {
        vec![Modality::Text]
    } else {
        parsed
...
fn map_model(row: &PgRow) -> LlmModelConfig {
    LlmModelConfig {
        ...
        name: row.get("name"),
        model: row.get("model"),
        input: parse_modalities(row.get("input_modalities")),
...
fn map_active(row: &PgRow) -> AppResult<ActiveLlm> {
    ...
    Ok(ActiveLlm {
        ...
        model: LlmModelConfig {
            ...
            input: parse_modalities(row.get("m_input_modalities")),
